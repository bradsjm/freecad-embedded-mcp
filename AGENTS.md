...
**Documents and objects.** Name sanitization and dedupe; save-as and save-to-own-file without consent; consent for dirty close/reload; `create_objects` creates 1–32 entries atomically and returns requested-to-actual `nameMapping`; `edit_objects` rolls the whole batch when a later entry is invalid; `response_detail: "compact"` keeps post-state reports while omitting before-state deltas; `delete_object` refuses dependents and names them, except a PartDesign dependent whose only link is the target's `BaseFeature` — the native removal clears that link (the dependent falls back to the previous solid feature), the response reports it in `rerouted`, and a reroute that changes the Body's solid count still rolls back through the gate; `expected_solids`/`expected_bounds` gates report exact deviations and `operationState: rolled_back`; null-shape bodies are valid with `solid_count: null`. `open_document` reports `alreadyOpen` when the path was already open (FreeCAD returns the live in-memory document instead of re-reading the file).
...
**`inspect_documents.editObject` is not an edit session.** No add-on tool opens a GUI edit session, so none needs to close one. The field reports `App::Document::getActiveObject()` — FreeCAD's last activated object, set by native `addObject(..., ActivateObject)` for every created object (a `run_script` `doc.addObject` sets it too) and cleared only when that object is removed. The session is `Gui.Document.getInEdit()`: null after any tool call (verified live on 1.1.3, including after `create_feature` and `import_model`), opened only by `setEdit` and cleared only by `resetEdit`, neither of which the add-on calls. Read `editObject` as "the document's active object", never as "an edit session is open".
