...
The server builds its tool surface from the registered definitions. `run_script` is registered but appears in `tools/list` only when the `allow_scripts` setting is enabled; a disabled call answers `METHOD_NOT_FOUND` before schema validation or dispatch.
...
- `examples/cantilever_fem.py` — manual, stdlib-only MCP client demo (not a test).
