...
The server registers 25 tools in a fixed order: `discover_capabilities`, `inspect_documents`, `new_document`, `open_document`, `import_model`, `save_document`, `close_document`, `reload_document`, `inspect_objects`, `create_object`, `edit_object`, `edit_objects`, `delete_object`, `validate_geometry`, `measure`, `inspect_topology`, `edit_parameters`, `inspect_sketch`, `edit_sketch`, `create_feature`, `edit_feature`, `export`, `capture_view`, `run_fem`, plus `run_script`, which appears in `tools/list` only when the `allow_scripts` setting is enabled (otherwise `tools/call` answers `METHOD_NOT_FOUND`).
...
- `mcp_server/server.py` (~2700 lines) — orchestrator: tool registry, dispatch, consent choreography, operation registry, deadlines, script-session cap, lifecycle.
...
**Consent (MRTR).** With a form-elicitation-capable legacy client: `elicitation/create` carries the fixed `confirmed` boolean form with target-specific text for open, overwrite-save, dirty close, dirty reload, and export overwrite; `accept` returns the final result on the same stream; `decline`, `cancel`, and a malformed response each end in `CONSENT_DENIED` with distinct detail reasons. A client without form support bypasses the prompt silently (documented 1.0 fallback).

**Documents and objects.** Name sanitization and dedupe; save-as and save-to-own-file without consent; consent for dirty close/reload; `edit_objects` rolls the whole batch when a later entry is invalid; `delete_object` refuses dependents and names them; `expected_solids`/`expected_bounds` gates report exact deviations and `operationState: rolled_back`; null-shape bodies are valid with `solid_count: null`. `open_document` reports `alreadyOpen` when the path was already open (FreeCAD returns the live in-memory document instead of re-reading the file).

**PartDesign and Sketcher.** `rectangle` composite expands to 4 segments + 8 auto constraints; explicit datums reach DoF 0; a padded 40×30×10 rectangle verifies at 12000 mm³; `fillet` from a signed edge reference; `gear_profile` wire; `hole` counterbore and `primitive` required-field validation; `properties`/`parameters` exclusivity. Crash-input point positions (≥ 3), unrecorded `(type, arity)` shapes, invalid datum strings, `expected_generation` mismatches, non-sketch targets, empty batches, negative radii, 2-sided polygons, and duplicate polyline points are all refused before any native call. Signed topology references and pagination cursors refuse stale generations and tampered signatures (`stale_generation`, `stale_cursor`, `malformed_cursor`).

**Query, export, view, import.** `measure` distance/interference/section/faces plus signed-reference selectors; a `box` selector selects the one face or edge whose document-space bounds equal it within tolerance (enclosing boxes deliberately match nothing — take bounds from `inspect_topology`). `export` STL/STEP/3MF verify by readback; paths outside `allowed_roots` are refused in preflight before any file effect; a missing parent directory produces an explicit error. `capture_view` honors explicit sizes and `focus_subelement` and rejects invalid `FaceN` with the face count. `import_model` STEP reports created objects with bounds and validity. Forty concurrent exports complete without reaching the 32-operation cap (they serialize on the GUI thread).
