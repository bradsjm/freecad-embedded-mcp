...
- `allowed_roots` (defaults to `~`) is path containment for file-touching tools, the FEM working directory, and the optional recovery directory only — not a sandbox, and it never constrains `run_script`.
