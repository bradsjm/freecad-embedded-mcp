...
with a session header, the initialized notification, the registered tools,
discover_capabilities and a harmless run_script as legacy-shaped final
results — plus the guarantee that modern requests still validate unchanged
and never acquire a legacy session implicitly.
"""
...
# ``test_server`` binds the stub tool modules and then releases the stub
# package, so these imports resolve the real modules.
...
def test_legacy_session_flow_delivers_final_results_over_sse(wired_server):
    ...
    # tools/list: registered tools, no modern envelope metadata.
    response = _post(
        http,
