...
- **Bounds are positional arrays.** `[xmin, ymin, zmin, xmax, ymax, zmax]` in document space — the bounding-box shape every geometry toolkit uses: compact, orderable, comparable, and the same shape in requests (`expected_bounds`), results, and selectors. Bounds describe the object's resulting `Shape` after placement: an additive or subtractive PartDesign feature reports the cumulative Body result, not the isolated material the feature added or removed, and compact and full detail agree.
