...
| `measure` | Measure distance, interference, difference (`a.cut(b)` — the material of `a` that `b` does not cover; an empty result reports `difference_volume` 0 with null bounds), section, and face relationships between objects, bbox-selected subshapes, or signed topology references. |
...
| `edit_feature` | Edit the native parameters of one existing pad, pocket, hole, or gear feature that belongs to the named Body — for the hole this includes its cut, depth-type and thread parameters — returning actual before/after values and Body validation. Unsupported feature types report the supported kinds and point to `edit_object`. |
...
`capture_view` is organized by inspection intent, not by camera names. `mode: "overview"` (the default) captures a labeled 4x2 sheet (2048x1104) of the `Isometric`, `Front`, `Back`, `Left`, `Right`, `Top`, and `Bottom` orientations plus a legend cell; without `focus_object` it frames the whole visible document and reports `captured_objects`, with `focus_object` it frames that object (optionally one validated `FaceN`/`EdgeN`). `mode: "detail"` captures one enlarged view of the focus target; without `view_name` the orientation derives from a planar `FaceN` target (camera direction opposite the face normal), and `view_name` alone (no `mode`) keeps the legacy explicit single-view capture. `mode: "interior"` captures a labeled 2x2 sheet (1024x1104): three mid-plane sections through the focus object via the native clipping plane plus one x-ray `Isometric` panel (focus `Transparency` 75 for that panel only). `mode: "fit"` captures a labeled 1x2 sheet (1024x552): two objects framed uncut plus a section through their mating axis, derived from one coaxial cylindrical face pair or given as `section_axis`/`section_point`. For sheets, explicit `width`/`height` scale the panels with the label strips constant (up to 4096 per side); single-panel captures honor explicit sizes and otherwise follow the active viewport scaled to a 768-pixel longest edge. The `views` manifest in `structuredContent` is authoritative for panel identity, camera direction/up, section planes, and panel rects. Section and x-ray panels are qualitative: occlusion and cut-surface appearance are not guaranteed, and `measure`/`inspect_topology` remain the authoritative evidence.
