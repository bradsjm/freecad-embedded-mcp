...
| `close_document` | Close a document and report its prior path and whether unsaved changes were discarded. |
| `reload_document` | Close and reopen a saved document from its file. |
| `inspect_objects` | List document objects, or a 1–64 object selection, with placement, bounds, shape validity, and solid counts. Results are paginated with a signed cursor and support compact/full detail and property pages. Full spreadsheet rows include bounded cells, aliases, formulas, values, and errors. |
...
| `edit_object` | Assign properties with prevalidation so an invalid property leaves earlier properties unchanged. `Spreadsheet::Sheet` cell contents use `properties.cells`, and the change summary reports `cellContentsPersisted` after recompute readback. Supports canonical link targets — a whole object (`{object}`), a signed reference (`{object, subelement}`), or a one-result query (`{object, query}`) — an optional `expected_generation` guard, optional commit-time bounds expectations, and `response_detail: "compact"` or `"full"` change reports. |
| `edit_objects` | Edit 1–32 existing objects atomically in one transaction with an optional top-level `expected_generation` guard, optional per-object expectations, and compact/full change reports. |
| `delete_object` | Remove an object, refusing objects that still have dependents instead of cascading silently. A PartDesign dependent whose only link is the target's `BaseFeature` is rerouted by the native removal and reported in `rerouted` (its cleared link reports `baseFeature: null`). |
...
| `measure` | Measure distance, interference, difference (`a.cut(b)` — the material of `a` that `b` does not cover; an empty result reports `difference_volume` 0 with null bounds), planar section, and face relationships between shared targets: a whole object (`{object}`), a signed reference (`{object, subelement}`), or a declarative query (`{object, query}`) that must resolve to exactly one subshape (`selection_empty` or `selection_ambiguous` otherwise). Query selectors use the shared CadQuery-style grammar; results carry `document` and `generation`, and `a`/`b` echo canonical targets. |
| `inspect_topology` | Page through the topology of one shared target: a whole object enumerates its faces, a query target evaluates the shared selector chain and reports the final-stage set (a final `edge` step reports edges), and a signed reference returns its one referenced subshape. Items carry native 1-based indices, bounds, native type names, and signed references; full detail adds sampled centers, normals, radii, axes, and lengths. The signed cursor is bound to identity, generation, object, role, page size, and the canonical target. |
| `edit_parameters` | Add, rename, bind expressions on, and clear expressions from dynamic properties with full validation and rollback. Results include a post-state `bodyReport` and explicit `units`; `response_detail: "full"` also returns the pre-edit report. Native dotted targets such as `Placement.Base.y` are supported when their root property exists. |
...
| `edit_sketch` | Apply one atomic batch of sketch operations: add geometry (including `rectangle`, `polyline`, `regularPolygon`, `slot`, and `rounded_rectangle` profiles), add constraints, set datums, bind constraint expressions, and delete geometry or constraints. `slot` requires an overall end-to-end `length` and a `diameter` with `length > diameter > 0` and expands to 4 geometry elements plus 5 constraints; `rounded_rectangle` requires `width`, `height`, and `corner_radius` (`0 < corner_radius < min(width, height)/2`) and expands to 8 plus 15. Both close their chain with endpoint-specific four-argument `Tangent` joins (`Tangent(geoA, 2, geoB, 1)`), which the FreeCAD 1.1.3 solver accepts without redundancy. |
...
| `edit_feature` | Edit the native parameters of one existing PartDesign feature in the named Body: `pad`, `pocket`, `hole` (including cut, depth-type, and thread parameters), `gear_profile`, `fillet` (`radius`), `chamfer` (`size`), `linear_pattern` (`count` 2–32, `length`), `polar_pattern` (`count` 2–32, `angle`), or `revolve` (`angle` in (0, 360], `reversed`). Parameters are validated against the selected kind's semantic names; positivity and angle limits hold after expression resolution, and a resolved value outside range refuses and rolls back. `parameterValues` reports each requested parameter's persisted expression and actual native value (`after` always, `before` with `response_detail: "full"`), and `bodyReport` always carries the post-state verdict. Unsupported feature types report the nine supported kinds and point to `edit_object`. |
| `export` | Write STL, STEP, 3MF, or a native FCStd copy, verifying every file by reading it back. |
| `capture_view` | Capture exactly one labeled PNG per inspection intent: `overview` (default) is a labeled 4x2 sheet of the seven named orientations plus a legend, `detail` is one enlarged view of a focus target, `interior` is mid-plane sections plus an x-ray panel, and `fit` is two objects uncut plus a section through their mating axis. The caller's selection, active document, camera, clipping plane, and focus transparency are restored. The PNG is published once in the image content block; structuredContent carries metadata only. |
...
`capture_view` is organized by inspection intent, not by camera names. `mode: "overview"` (the default) captures a labeled 4x2 sheet (2048x1104) of the `Isometric`, `Front`, `Back`, `Left`, `Right`, `Top`, and `Bottom` orientations plus a legend cell; without `focus` it frames the whole visible document and reports `captured_objects`, with `focus` it frames that shared target (a whole object, a signed reference, or a query resolved to one subshape). `mode: "detail"` captures one enlarged view of the focus target; without `view_name` the orientation derives from a planar face target (camera direction opposite the face normal), and `view_name` is accepted only with `mode: "detail"`, where it selects the explicit single-view capture — another mode carrying `view_name` is refused with `invalid_parameter_for_mode`. `mode: "interior"` captures a labeled 2x2 sheet (1024x1104): three mid-plane sections through a whole-object focus via the native clipping plane plus one x-ray `Isometric` panel (focus `Transparency` 75 for that panel only); a subshape focus is refused. `mode: "fit"` captures a labeled 2x1 sheet (1024x552): two shared targets framed uncut plus a section through their mating axis, derived from one coaxial cylindrical face pair or given as `section_axis`/`section_point`; fit refuses `focus`. For sheets, explicit `width`/`height` scale the panels with the label strips constant (up to 4096 per side); single-panel captures honor explicit sizes and otherwise follow the active viewport scaled to a 768-pixel longest edge. The `views` manifest in `structuredContent` is authoritative for panel identity, camera direction/up, section planes, and panel rects, and the result reports resolved `focus`/`a`/`b` targets plus document generation. Section and x-ray panels are qualitative: occlusion and cut-surface appearance are not guaranteed, and `measure`/`inspect_topology` remain the authoritative evidence.
...
`response_detail` defaults to `"compact"` on object and feature mutations; pass `"full"` explicitly for before/after deltas. Compact changes omit before-state property and geometry deltas and dependent counts, but keep the post-state validation report.
