...
Use this skill only when a language model is driving a live FreeCAD `1.1.3 <= version < 1.2` installation through the `freecad-mcp` MCP server. All operations in this skill must be performed through the embedded server's MCP tools; `run_script` is the arbitrary-Python path and executes on the FreeCAD GUI thread. A Tasks-capable client may receive a task id for it, but the code remains cooperative after it starts. Do not instruct a human to operate FreeCAD or a website.
...
1. **Check server health.** Call `discover_capabilities` first; it never waits for the GUI thread. Read `gui.state`. If it is not healthy, do not issue more GUI-thread operations. Call `tools/list` when you need exact tool schemas. Call `discover_capabilities` again after a suspected timeout.
...
7. **Build in dependency order.** Create base geometry, references/sketches/features, cuts/dress-ups/patterns. `create_object`, `create_objects`, `edit_object`, and `edit_objects` recompute inside MCP-owned transactions and validate dependents and solid counts. Inspect returned names and states after each meaningful stage.
   Save a checkpoint after each validated modeling stage, before expensive checks or cleanup. When `capabilities.recoveryEnabled` is true, expensive feature mutations also create verified FCStd recovery copies before their transactions open. Run parameter experiments in a separate validation copy. Change, check, and restore one parameter at a time. See [checkpoint and crash recovery](references/troubleshooting.md#checkpoint-and-crash-recovery).
...
9. **Keep document and GUI work on the GUI thread.** Use the structured tools for their covered operations: `discover_capabilities`, `inspect_documents`, `new_document`, `open_document`, `import_model`, `save_document`, `close_document`, `reload_document`, `inspect_objects`, `create_object`, `create_objects`, `edit_object`, `edit_objects`, `edit_parameters`, `delete_object`, `validate_geometry`, `measure`, `inspect_topology`, `inspect_sketch`, `edit_sketch`, `create_feature`, `edit_feature`, `export`, `capture_view`, and `run_fem`. Use `run_script` for `FreeCADGui`, selection, mesh formats `import_model` does not support, and anything else the structured tools do not cover. `run_fem`, `run_script`, `export`, and `measure` may detach as Tasks; poll `tasks/get`, and treat the terminal task result as the only completion signal. Use `resources/list` and `resources/read` for the live document resource, and use `subscriptions/listen` for document or task update streams when the client supports them.
...
11. **Export the result.** Save the editable source with `save_document`. Treat a successful `save_document` result as the save acknowledgement. Use the `export` tool for STL, STEP, 3MF, or a native FCStd copy. It writes to a temporary sibling file, verifies the result by readback, and requires consent to overwrite an existing destination. Report the readback values. `close_document` never saves; it reports the prior path and whether it discarded unsaved changes. Reopen and inspect the file when an independent persistence check is required.
...
- **Independent object batch:** use `create_objects` for 1–32 independent objects in one transaction. Read its `nameMapping`, then use `edit_objects` for links that need the actual names.
