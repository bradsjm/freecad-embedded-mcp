# English FreeCAD wiki navigation

Use only the English canonical URLs below. These links are durable references to the FreeCAD wiki; do not cite the temporary local documentation clone in answers.

## Start and FreeCAD 1.1

- [Getting started](https://wiki.freecad.org/Getting_started)
- [Document structure](https://wiki.freecad.org/Document_structure)
- [Workbenches](https://wiki.freecad.org/Workbenches)
- [Release notes 1.1](https://wiki.freecad.org/Release_notes_1.1)
- [Property](https://wiki.freecad.org/Property)
- [Property editor](https://wiki.freecad.org/Property_editor)
- [Placement](https://wiki.freecad.org/Placement)
- [Part Attachment](https://wiki.freecad.org/Part_EditAttachment)
- [Mouse navigation](https://wiki.freecad.org/Mouse_navigation)
- [3D View](https://wiki.freecad.org/3D_View)

## Part and geometry

- [Part Workbench](https://wiki.freecad.org/Part_Workbench)
- [Part scripting](https://wiki.freecad.org/Part_scripting)
- [Topological data scripting](https://wiki.freecad.org/Topological_data_scripting)
- [Part Primitives](https://wiki.freecad.org/Part_Primitives)
- [Part Box](https://wiki.freecad.org/Part_Box)
- [Part Cylinder](https://wiki.freecad.org/Part_Cylinder)
- [Part Sphere](https://wiki.freecad.org/Part_Sphere)
- [Part Cone](https://wiki.freecad.org/Part_Cone)
- [Part Torus](https://wiki.freecad.org/Part_Torus)
- [Part Wedge](https://wiki.freecad.org/Part_Wedge)
- [Part Extrude](https://wiki.freecad.org/Part_Extrude)
- [Part Revolve](https://wiki.freecad.org/Part_Revolve)
- [Part Loft](https://wiki.freecad.org/Part_Loft)
- [Part Sweep](https://wiki.freecad.org/Part_Sweep)
- [Part Boolean](https://wiki.freecad.org/Part_Boolean)
- [Part Cut](https://wiki.freecad.org/Part_Cut)
- [Part Fuse](https://wiki.freecad.org/Part_Fuse)
- [Part Common](https://wiki.freecad.org/Part_Common)
- [Part Fillet](https://wiki.freecad.org/Part_Fillet)
- [Part Chamfer](https://wiki.freecad.org/Part_Chamfer)
- [Part Thickness](https://wiki.freecad.org/Part_Thickness)
- [Part RefineShape](https://wiki.freecad.org/Part_RefineShape)
- [Part CheckGeometry](https://wiki.freecad.org/Part_CheckGeometry)

## PartDesign and Sketcher

- [PartDesign Workbench](https://wiki.freecad.org/PartDesign_Workbench)
- [PartDesign Body](https://wiki.freecad.org/PartDesign_Body)
- [Feature editing](https://wiki.freecad.org/Feature_editing)
- [PartDesign NewSketch](https://wiki.freecad.org/PartDesign_NewSketch)
- [PartDesign Pad](https://wiki.freecad.org/PartDesign_Pad)
- [PartDesign Pocket](https://wiki.freecad.org/PartDesign_Pocket)
- [PartDesign Revolution](https://wiki.freecad.org/PartDesign_Revolution)
- [PartDesign Hole](https://wiki.freecad.org/PartDesign_Hole)
- [PartDesign Fillet](https://wiki.freecad.org/PartDesign_Fillet)
- [PartDesign Chamfer](https://wiki.freecad.org/PartDesign_Chamfer)
- [PartDesign Mirrored](https://wiki.freecad.org/PartDesign_Mirrored)
- [PartDesign LinearPattern](https://wiki.freecad.org/PartDesign_LinearPattern)
- [PartDesign PolarPattern](https://wiki.freecad.org/PartDesign_PolarPattern)
- [PartDesign ShapeBinder](https://wiki.freecad.org/PartDesign_ShapeBinder)
- [Sketcher Workbench](https://wiki.freecad.org/Sketcher_Workbench)
- [Sketcher scripting](https://wiki.freecad.org/Sketcher_scripting)
- [Sketcher SketchObject](https://wiki.freecad.org/Sketcher_SketchObject)
- [Sketcher NewSketch](https://wiki.freecad.org/Sketcher_NewSketch)
- [Sketcher CreateLine](https://wiki.freecad.org/Sketcher_CreateLine)
- [Sketcher CreateCircle](https://wiki.freecad.org/Sketcher_CreateCircle)
- [Sketcher CreateRectangle](https://wiki.freecad.org/Sketcher_CreateRectangle)
- [Sketcher ConstrainCoincident](https://wiki.freecad.org/Sketcher_ConstrainCoincident)
- [Sketcher ConstrainDistance](https://wiki.freecad.org/Sketcher_ConstrainDistance)
- [Sketcher ConstrainDistanceX](https://wiki.freecad.org/Sketcher_ConstrainDistanceX)
- [Sketcher ConstrainDistanceY](https://wiki.freecad.org/Sketcher_ConstrainDistanceY)
- [Sketcher ConstrainRadius](https://wiki.freecad.org/Sketcher_ConstrainRadius)
- [Sketcher ConstrainDiameter](https://wiki.freecad.org/Sketcher_ConstrainDiameter)
- [Sketcher ConstrainAngle](https://wiki.freecad.org/Sketcher_ConstrainAngle)
- [Sketcher ConstrainHorizontal](https://wiki.freecad.org/Sketcher_ConstrainHorizontal)
- [Sketcher ConstrainVertical](https://wiki.freecad.org/Sketcher_ConstrainVertical)
- [Sketcher ConstrainParallel](https://wiki.freecad.org/Sketcher_ConstrainParallel)
- [Sketcher ConstrainPerpendicular](https://wiki.freecad.org/Sketcher_ConstrainPerpendicular)
- [Sketcher ConstrainTangent](https://wiki.freecad.org/Sketcher_ConstrainTangent)
- [Sketcher ConstrainEqual](https://wiki.freecad.org/Sketcher_ConstrainEqual)
- [Sketcher ConstrainSymmetric](https://wiki.freecad.org/Sketcher_ConstrainSymmetric)
- [Sketcher ConstrainPointOnObject](https://wiki.freecad.org/Sketcher_ConstrainPointOnObject)
- [Sketcher ConstrainBlock](https://wiki.freecad.org/Sketcher_ConstrainBlock)
- [Sketcher ValidateSketch](https://wiki.freecad.org/Sketcher_ValidateSketch)

## Draft

- [Draft Workbench](https://wiki.freecad.org/Draft_Workbench)
- [Draft scripting/API](https://wiki.freecad.org/Draft_API)
- [Draft Circle](https://wiki.freecad.org/Draft_Circle)
- [Draft Rectangle](https://wiki.freecad.org/Draft_Rectangle)
- [Draft Polygon](https://wiki.freecad.org/Draft_Polygon)
- [Draft Wire](https://wiki.freecad.org/Draft_Wire)
- [Draft BSpline](https://wiki.freecad.org/Draft_BSpline)
- [Draft ShapeString](https://wiki.freecad.org/Draft_ShapeString)
- [Draft Move](https://wiki.freecad.org/Draft_Move)
- [Draft Rotate](https://wiki.freecad.org/Draft_Rotate)
- [Draft Scale](https://wiki.freecad.org/Draft_Scale)
- [Draft Array](https://wiki.freecad.org/Draft_Array)
- [Draft WorkingPlane](https://wiki.freecad.org/Draft_WorkingPlane)

`Draft_API` is useful as a navigation pointer but is explicitly marked outdated by the wiki; verify APIs in the running FreeCAD 1.1 Python help and the current source.

## FEM

- [FEM Workbench](https://wiki.freecad.org/FEM_Workbench)
- [FEM Analysis](https://wiki.freecad.org/FEM_Analysis)
- [FEM Material Solid](https://wiki.freecad.org/FEM_MaterialSolid)
- [FEM Mesh](https://wiki.freecad.org/FEM_Mesh)
- [FEM Mesh Gmsh From Shape](https://wiki.freecad.org/FEM_MeshGmshFromShape)
- [FEM Mesh Region](https://wiki.freecad.org/FEM_MeshRegion)
- [FEM Geometry Preparation and Meshing](https://wiki.freecad.org/FEM_Geometry_Preparation_and_Meshing)
- [FEM Constraint Fixed](https://wiki.freecad.org/FEM_ConstraintFixed)
- [FEM Constraint Force](https://wiki.freecad.org/FEM_ConstraintForce)
- [FEM Constraint Pressure](https://wiki.freecad.org/FEM_ConstraintPressure)
- [FEM Solver CalculiX](https://wiki.freecad.org/FEM_SolverCalculixCcxtools)
- [FEM Solver Run](https://wiki.freecad.org/FEM_SolverRun)
- [FEM Result Show](https://wiki.freecad.org/FEM_ResultShow)
- [CalculiX](https://wiki.freecad.org/FEM_CalculiX)

## Mesh, export, and repair

- [Mesh Workbench](https://wiki.freecad.org/Mesh_Workbench)
- [Mesh from Part Shape](https://wiki.freecad.org/Mesh_FromPartShape)
- [Mesh Scripting](https://wiki.freecad.org/Mesh_Scripting)
- [Mesh to Part](https://wiki.freecad.org/Mesh_to_Part)
- [Part Shape From Mesh](https://wiki.freecad.org/Part_ShapeFromMesh)
- [Part MakeSolid](https://wiki.freecad.org/Part_MakeSolid)
- [Export to STL or OBJ](https://wiki.freecad.org/Export_to_STL_or_OBJ)
- [Standard Export](https://wiki.freecad.org/Std_Export)
- [Import Export](https://wiki.freecad.org/Import_Export)
- [Import/Export Preferences](https://wiki.freecad.org/Import_Export_Preferences)
- [Mesh Evaluation](https://wiki.freecad.org/Mesh_Evaluation)

The STL tutorial is useful for units and tessellation concepts but predates FreeCAD 1.1 menus. Verify current exporter/tessellation behavior programmatically in the live installation with `FreeCAD.getExporters()`, `dir(MeshPart)`, and `supportedTypes()`.

## Python and add-ons

- [FreeCAD Scripting Basics](https://wiki.freecad.org/FreeCAD_Scripting_Basics)
- [Python](https://wiki.freecad.org/Python)
- [Python scripting tutorial](https://wiki.freecad.org/Python_scripting_tutorial)
- [Scripting and macros](https://wiki.freecad.org/Scripting_and_macros)
- [Macros](https://wiki.freecad.org/Macros)
- [Power users hub](https://wiki.freecad.org/Power_users_hub)
- [Addon Manager](https://wiki.freecad.org/Std_AddonMgr)
- [Parts Library](https://wiki.freecad.org/Parts_Library)
- [Parts Library Workbench](https://wiki.freecad.org/Parts_Library_Workbench)

## Scope exclusions

The MCP tools do not expose a CAM toolpath generator or a TechDraw workflow. `export` covers STL, STEP, 3MF, and FCStd output; other import/export routes may be reached indirectly through `run_script`, but they are not direct MCP capabilities. Addon installation through the Addon Manager, if required, is an external/user-side boundary unless a safe non-interactive API has been verified in the running installation.
